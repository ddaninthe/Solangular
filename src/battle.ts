...
import { Attack, Nature } from "./attack";
...
export class Battle {
    ...
    private static attack(attacker: Pokemon, receiver: Pokemon): void {
        ...
        if (Battle.isAttackSuccessful(attack)) {
            ...
            if (receiver.life <= 0) { 
                receiver.life = 0; // Remise à 0 pour ne pas afficher de points négatifs
    ...
    public static fight(pokemon: Pokemon, pokemon2: Pokemon): void {
        console.log("Le combat commence.");

        this.rounds(pokemon, pokemon2)
            .then((winner: Pokemon) => {
